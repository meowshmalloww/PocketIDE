...
Java_org_nehuatl_llamacpp_LlamaContext_initContextWithFd(
    ...
    // A bare decimal descriptor only works in lm_ggml_fopen. Modern llama.cpp also opens the
    // model through POSIX open() while preparing mmap/direct-I/O, where "97" is treated as a
    // relative filename. /proc/self/fd keeps the descriptor contract while working through
    // every loader path and gives each open an independent file position.
    char fdPath[64];
    snprintf(fdPath, sizeof(fdPath), "/proc/self/fd/%d", dupfd);
    defaultParams.model.path = fdPath;
    ...
    int dup_mmproj_fd = -1;
    if (mmproj_fd >= 0) {
        dup_mmproj_fd = dup(mmproj_fd);
        if (dup_mmproj_fd != -1) {
            char mmproj_path[64];
            snprintf(mmproj_path, sizeof(mmproj_path), "/proc/self/fd/%d", dup_mmproj_fd);
    ...
    defaultParams.cache_type_k = rnllama::kv_cache_type_from_str(
        cache_type_k_chars != nullptr ? cache_type_k_chars : "f16");
    defaultParams.cache_type_v = rnllama::kv_cache_type_from_str(
        cache_type_v_chars != nullptr ? cache_type_v_chars : "f16");
    defaultParams.flash_attn_type = rnllama::flash_attn_type_from_str(
        flash_attn_type_chars != nullptr ? flash_attn_type_chars : "auto");

    auto llama = new rnllama::llama_rn_context();
    bool ok = llama->loadModel(defaultParams);
    // common_init_from_params has completed all model reads and mmap setup. The mapping stays
    // valid after the descriptor closes, and repeated profile reloads must not leak one FD each.
    close(dupfd);
    ...
    if (dup_mmproj_fd >= 0) close(dup_mmproj_fd);

    env->ReleaseStringUTFChars(lora_str, lora_chars);
    if (cache_type_k_chars != nullptr) env->ReleaseStringUTFChars(cache_type_k, cache_type_k_chars);
    if (cache_type_v_chars != nullptr) env->ReleaseStringUTFChars(cache_type_v, cache_type_v_chars);
    if (flash_attn_type_chars != nullptr) env->ReleaseStringUTFChars(flash_attn_type, flash_attn_type_chars);
...
Java_org_nehuatl_llamacpp_LlamaContext_loadModelDetails(
    ...
    putIntHashMap(env, result, "actual_context", (int) llama_n_ctx(llama->ctx));
    putIntHashMap(env, result, "actual_context_per_sequence", (int) llama_n_ctx_seq(llama->ctx));
    putIntHashMap(env, result, "actual_batch", (int) llama_n_batch(llama->ctx));
    putIntHashMap(env, result, "actual_micro_batch", (int) llama_n_ubatch(llama->ctx));
    putStringHashMap(env, result, "kv_cache_type_k", lm_ggml_type_name(llama->params.cache_type_k));
    putStringHashMap(env, result, "kv_cache_type_v", lm_ggml_type_name(llama->params.cache_type_v));
    putStringHashMap(env, result, "flash_attention", llama_flash_attn_type_name(llama->params.flash_attn_type));
